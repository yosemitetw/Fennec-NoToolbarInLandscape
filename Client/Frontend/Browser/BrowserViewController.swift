...
private struct BrowserLandscapeModeViewControllerUX {
    fileprivate static let BackgroundColor = UIConstants.AppBackgroundColor
    fileprivate static let ShowHeaderTapAreaHeight: CGFloat = 0
    fileprivate static let BookmarkStarAnimationDuration: Double = 0.5
    fileprivate static let BookmarkStarAnimationOffset: CGFloat = 80
}
...
class BrowserViewController: UIViewController {
    ...
    override func updateViewConstraints() {
        ...
        if UIDeviceOrientationIsPortrait( UIDevice.current.orientation ) {
            ...
            topTouchArea.snp.remakeConstraints { make in
                make.top.left.right.equalTo(self.view)
                make.height.equalTo(BrowserLandscapeModeViewControllerUX.ShowHeaderTapAreaHeight)
            }
        }
    ...
    func performMenuAction(_ action: MenuAction, withAppState appState: AppState) {
        if let menuAction = AppMenuAction(rawValue: action.action) {
            ...
            case .sharePage:
                ...
                // NEW MENU ITEM ACTION IMPLEMENTATION HERE !! NOTE !! WARNING
            case .reloadAuth:
                let thisURL = URL( string:"http://www.google.com" )
                self.openURLInNewTab(thisURL, isPrivate: false, isPrivileged: false)
                scrollController.hideToolbars( animated: true )
            default: break
            }
        }
    }
    // Jamen Request: Hide toolbar when it changed to landscape gesture.
